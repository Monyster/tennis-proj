...
import { MIN_PLAYERS, type Room } from '@/types';
...
export function Lobby({
  ...
  const playersInTeams = new Set(teams.flatMap((t) => [t.player1Id, t.player2Id]));
  ...
  return (
    <div className="max-w-2xl mx-auto p-6 space-y-6">
            ...
            return (
              <div
                ...
                className={`p-4 rounded-lg border-2 ${
                  isMyTeam ? 'border-blue-500 bg-blue-50' : 'border-gray-300 bg-white'
                }`}
              >
                ...
                {isMyTeam && <p className="text-center text-xs text-blue-600 mt-1">Ваша команда</p>}
              </div>
            );
          })}
        </div>
      ...
      {soloPlayers.length > 0 && (
        <div className="space-y-3">
          <h2 className="text-lg font-semibold text-gray-900">Гравці без пари</h2>
          {soloPlayers.map((player) => (
            <PlayerCard
              ...
              onInvite={
                player.id !== playerId && !myPartnerId ? () => onSendInvite(player.id) : undefined
              }
      ...
      {benchPlayers.length > 0 && (
        <div className="space-y-3">
          ...
          {benchPlayers.map((player) => (
            <PlayerCard key={player.id} player={player} isCurrentPlayer={player.id === playerId} />
          ))}
        </div>
      )}
      ...
      {!isHost && (
        <p className="text-center text-sm text-gray-600">Очікування на хоста для початку гри...</p>
      )}
