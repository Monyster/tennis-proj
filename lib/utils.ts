...
import { getServingTeam, type Player, type RotationDecision, type Team } from '@/types';
 ...
 */
export function whoStays(team: Team, players: Record<string, Player>): RotationDecision {
...
export function createRandomTeams(playerIds: string[]): TeamFormationResult {
  ...
  const playersToTeam = shuffled.length % 2 === 0 ? shuffled : shuffled.slice(0, -1);
...
export function getServingTeamText(winStreak: number): string {
  ...
  return servingTeam === 'champions' ? 'Подають чемпіони' : 'Подають претенденти';
}
